...
Данный драйвер считаю лучшим по оттенкам и цветам он не теряет даже в темноте дальше у фирмы каноникал идут драйверы цифрой выше , но это одно и то же что взять запорожец и наклеить на него лого ламборджини , мерседеса или ещё чего классом выше, но уже такого восприятия цветовой тональности уже нету , в браузере выставите флаг srgb и смотрите видео в лучшем цветовом пространстве и чёткости и не замыливании изображения. Вероятно этого человека если кто попросит https://habr.com/ru/post/440566/ может он сможет инструкции и всю эту цветность дёрнуть через гидру которые в моём драйвере , да ещё и может ускорить , но я не совсеми могу разговаривать я на взгляд перебрал много драйверов и могу только доказать что зрение у меня по лучше чем обращение со всякими регистрами драйверов потому что я не учился на это.
...
Остается только провести эксперимент выставить в root sRGB профиль монитора и пересобрать драйвер и тогда возможно он и получит эти характеристики попутно взятые с дампа компилятора , но делать я этого не буду потому что я уже сделал драйвер и вот он тут и является ли это той серебренной пулей в компиляции не знаю , а делал ли я таким образом пребилдинг с этими настройками уже не помню , и выбирать вам пользоваться видео драйвером от запорожца с наклеенной циферкой выше или оставаться на аурусе уже вам. На всякий случай я приготовил glibc-2.31 https://github.com/Griggorii/libc6-2.31-0ubuntu10-i386-x32-amd64.amd64.deb на случай если убунту уедет на новый glibc хотя они не уехали с него начиная с 19.04-20.04-20.10 и у меня поедет финансирование то мы можем продолжить не теряя этот графический стек. Ещё не плохо было бы вернуть версию gtk3 которая была в 18.10 в ней ещё не испорченный css стиль при переключений тем там такого не было что черные буквы на черном фоне то что сделали дальше это была не оправданная погоня каноникла за хайпом в итоге получили сломанный css стиль. А , ещё лучше у меня есть хорошо сложенная система на 16.04 на которой собирается qt4.8 её поднять до gtk3 только не грязными хаками намостив один стиль на другой , а чистыми точечными и есть будет 700-800 мб да и ещё очень быстрой , вся проблема что торвальдс занял всю ветку финансирования и какая то часть акции принадлежит этому самозванцу. Как мы вернём linux скорость которая была выше виндовой я сам всё решу , но на примете уже есть идея оцепить весь питон и вернуть qt4.8.7 туда встройть gtk тёмный , а точнее всё это есть и надо будет просто пилить gnome3 на qt4 , а позднее воткнуть пременную в /etc/environment - > export QT_QPA_PLATFORMTHEME=qt4 либо ещё легче export QT_QPA_PLATFORMTHEME=gtk2 потом придумать механизм где новые фаилы при компиляции будут не только в контеинер уходить , но и за него таким образом получим чистую структуру выбрасывая контеинер смешанных дистрибутивов тем самым вернём libpng12 по умолчанию <- по простой причине он универсальнее libpng16 и на нём работают программы как от qt4 так и от qt5 по этому в свой видео драйвер я включил именно libpng12 , пример программы которая зависит от libpng12 например SP Flash Tool v5 как раз таки нужная не игрушка что бы прошить мобильник бац и на libpng16 она внезапно не запускается. Есть мнение что торвальдс портит линукс поскольку все же не разбирается в его составных частях , но те кто застал мой линукс когда ещё я раздавал 16.04 то он летел до добавления туда ппа где всё таки прилетели некие пакеты сеошнего направления как в андроиде рпк сервисная синхронизация и взятие слепков бекэнда.
